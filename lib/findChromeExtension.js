...
function findChromeExtension(name, extId, browser) {
	...
	if (extPathMap[extId] && extId && extPathMap[extId].versionsDir) {
		// console.log('---' ,extPathMap[extId].versionsDir)
		extPathMap[extId].versionsDir.sort((a, b) => {
			const aTime = fs.statSync(a).mtime.getTime();
			const bTime = fs.statSync(b).mtime.getTime();
			return bTime - aTime;
		})
		// console.log('extPathMap[extId].versionsDir[0]---', extPathMap[extId].versionsDir[0])
		return extPathMap[extId].versionsDir[0];
