...
function findPreferences() {
	...
	for (const name of names) {
		['Secure Preferences', 'Preferences'].forEach(item => {
			const abspath = path.join(chrome, name, item);
			// const abspath = path.join(chrome, name, 'Preferences');
			// console.log(abspath)
			// if ((name === 'Preferences') && isFile(abspath))
